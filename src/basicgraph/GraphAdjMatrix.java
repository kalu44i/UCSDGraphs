...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		//result matrix has dimensions as an adjMatrix
		int[][] resultMatrix = new int[adjMatrix.length][adjMatrix.length];
		//loop in rows
		for (int i = 0 ; i < adjMatrix.length; i++) {
			//loop in columns
			for (int j = 0; j < adjMatrix.length; j++) {
				int result = 0;
				//do mutiplication of row and column
				for (int c = 0; c < adjMatrix.length; c++) {
					result = result + adjMatrix[i][c]*adjMatrix[c][j];
				}
		...
		for (int i = 0; i < resultMatrix.length; i++) {
			if (resultMatrix[v][i] == 1) {
				list.add(i);
			} else if (resultMatrix[v][i] > 1) {
				for (int j = 0; j < resultMatrix[v][i]; j++) {
					list.add(i);
				}
			}
		}
